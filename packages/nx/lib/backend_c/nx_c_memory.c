...
static value make_contiguous(value v_src, bool force_copy) {
  ...
  if (can_share) {
    ...
    /* do not propagate internal flags, like CAML_BA_SUBARRAY, that can lead to OOM */
    v_new_data = caml_ba_alloc(flags & (CAML_BA_KIND_MASK | CAML_BA_LAYOUT_MASK | CAML_BA_MANAGED_MASK), 1, NULL, dims);
    v_new_shape = copy_int_array(Field(v_src, FFI_TENSOR_SHAPE));
